...
export function BurgerMenu({
    ...
    return (
        <div className="status-menu-popover" role="menu" aria-label="App menu">
            ...
            <button type="button" className="status-menu-item" onClick={() => WindowReload()}>Reload app</button>
            ...
            <button type="button" className="status-menu-item" onClick={() => BrowserOpenURL(HELP_SITE_URL)}>Open Full Help Site</button>
            <button type="button" className="status-menu-item" onClick={onOpenAIDebug}>
                AI Debug Log{aiDebugLogCount > 0 ? ` (${aiDebugLogCount})` : ''}
            </button>
            <button type="button" className="status-menu-item" onClick={() => BrowserOpenURL('https://github.com/Zaitsev/run-calc')}>GitHub</button>
            <button type="button" className="status-menu-item" onClick={() => alert('Run-Calc is a native Wails desktop calculator with a system menu and standard OS window chrome.')}>About</button>
            <button type="button" className="status-menu-item status-menu-item--danger" onClick={() => Quit()}>Quit (your work is saved)</button>
        </div>
    );
}
